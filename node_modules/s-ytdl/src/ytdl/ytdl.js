...
const Ytdl = {
    ...
    async dl(link, qualityIndex, typeIndex) {
        ...
        const quality = Ytdl.qualities[type][qualityIndex];
        if (!type) throw new Error('❌ Tipe tidak valid. Pilih 1 untuk audio atau 2 untuk video.');
        Ytdl.checkQuality(type, qualityIndex);
        const cdnNumber = Ytdl.cdn();
        const cdnUrl = `cdn${cdnNumber}.savetube.su`;
        
        const videoInfo = await Ytdl.fetchData(`https://${cdnUrl}/info`, cdnNumber, { url: link });
        const badi = {
            downloadType: type,
            quality: quality,
        ...
        const dlRes = await Ytdl.fetchData(Ytdl.dLink(cdnUrl, type, quality, videoInfo.data.key), cdnNumber, badi);
